...
object CommonMigrations {
    val MIGRATION_92_93 = object : Migration(92, 93) {
        override suspend fun migrate(connection: SQLiteConnection) {
            ...
            // TasksJsonImporter.reconstructDirtyVersion(lastSync, modified) — keep the two in sync.
            connection.execSQL("""
                ...
                WHERE ct.`cd_deleted` = 0
                  AND (
                    SELECT `cda_account_type` FROM `caldav_lists`
                    INNER JOIN `caldav_accounts` ON `cda_uuid` = `cdl_account`
                    WHERE `cdl_uuid` = ct.`cd_calendar`
                    LIMIT 1
                  ) IS NOT $TYPE_LOCAL
            """.trimIndent())
        }
    ...
    val MIGRATION_98_99 = object : Migration(98, 99) {
        override suspend fun migrate(connection: SQLiteConnection) {
            connection.execSQL("ALTER TABLE `caldav_lists` ADD COLUMN `cdl_notifications_enabled` INTEGER NOT NULL DEFAULT 1")
        }
    }
