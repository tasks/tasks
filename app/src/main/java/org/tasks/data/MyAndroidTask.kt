...
class MyAndroidTask() : DmfsTask(null) {
    ...
    fun toBuilder(uri: Uri): BatchOperation.CpoBuilder {
        ...
        if (!isNew) {
            builder.removeValueBackReference(TaskContract.Tasks._UID)
        }
        return builder
                .removeValueBackReference(TaskContract.Tasks.CREATED)
                .removeValueBackReference(TaskContract.Tasks.LAST_MODIFIED)
                .removeValueBackReference(TaskContract.Tasks._DIRTY)
                .removeValueBackReference(TaskContract.Tasks.SYNC_VERSION)
    }
